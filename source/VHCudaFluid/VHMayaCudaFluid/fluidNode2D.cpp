...
void fluidNode2D::draw( M3dView & view, const MDagPath & path, 
	...
	MPlug sizeXPlug( thisNode, aSizeX );
	float sizeX;
	sizeXPlug.getValue(sizeX);
	sizeX = sizeX*0.5;

	MPlug sizeYPlug( thisNode, aSizeY );
	float sizeY;
	sizeYPlug.getValue(sizeY);
	sizeY=sizeY*0.5;
	...
	if(preview) {
		...
		if ( ( style == M3dView::kFlatShaded ) ||  ( style == M3dView::kGouraudShaded ) ) {  
			...
			int previewType;
			previewTypePlug.getValue(previewType);

			MPlug maxBoundsPlug( thisNode, aMaxBounds );
			float maxBounds;
			maxBoundsPlug.getValue(maxBounds);

			if (fluidInitialized)
				renderFluid(&fluidSolver,d_output,previewType,maxBounds);
			...
			glBegin( GL_QUADS );
			glTexCoord3f(0.0f, 0.0f, 0.0f); glVertex3f(-sizeX,-sizeY,0.0f);
			glTexCoord3f(0.0f, 1.0f, 0.0f); glVertex3f(-sizeX,sizeY,0.0f);
			glTexCoord3f(1.0f, 1.0f, 0.0f); glVertex3f(sizeX,sizeY,0.0f);
			glTexCoord3f(1.0f, 0.0f, 0.0f); glVertex3f(sizeX,-sizeY,0.0f);
			glEnd();
...
void fluidNode2D::initPixelBuffer(){
...
fluidNode2D::fluidNode2D() {
	...
	fluidSolver.res = cu::make_int2(-1,-1);
	...
	fluidSolver.borderNegX = 1;
	fluidSolver.borderPosX = 1;
	fluidSolver.borderNegY = 1;
	fluidSolver.borderPosY = 1;
...
void fluidNode2D::changeFluidRes(int x, int y) {


	clearFluid(&fluidSolver);
	initFluid(&fluidSolver,x,y);
	displayX = x;
	displayY = y;
	initPixelBuffer();
	...
	MStatus returnStatus;
	 ...
	 if(plug == aOutTime) {
		...
		if (newResX != fluidSolver.res.x || newResY != fluidSolver.res.y) {
			changeFluidRes(newResX, newResY);
		}
		...
		if (currentFrame <= startFrame) {
			...
			resetFluid(&fluidSolver);

			if (preview == false && fluidConnected)
